(() => {
  ...
  const URL_KEY = 'meta_shopee_script_url';
  const SHEETS_KEY = 'meta_shopee_sheets_data_v4';
  const TOKEN_KEY = 'meta_shopee_secret_token';
  const LIMIT = 10;
  const STATUSES = ['SCALE', 'HOLD', 'WATCH', 'KILL'];

  let entries = parse(localStorage.getItem(STORE), []);
  let sheets = parse(localStorage.getItem(SHEETS_KEY), []);
  let scriptUrl = localStorage.getItem(URL_KEY) || '';
  let secret = localStorage.getItem(TOKEN_KEY) || '';
  let range = '7days';
  let showAll = false;
  let chart = null;
  let editing = null;
  ...
  function dte(value) { return value ? new Date(value + 'T00:00:00').toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'; }
  ...
  function init() {
    ...
    on('btn-reset', reset);
    ['filter-product', 'filter-campaign', 'filter-source', 'filter-status'].forEach((id) => on(id, render, 'change'));
  ...
  function on(id, handler, event = 'click') { $(id).addEventListener(event, handler); }
  ...
  function token() { return (secret || localStorage.getItem(TOKEN_KEY) || '').trim(); }

  async function post(data) {
    const currentToken = token();
    if (!currentToken) throw new Error('Token belum diisi');
    const response = await fetch(scriptUrl, { method: 'POST', body: JSON.stringify({ ...data, token: currentToken }), headers: { 'Content-Type': 'text/plain' } });
  ...
  function formEntry() {
    const entry = norm({
      id: editing?.id || uuid(), rowId: editing?.rowId || '', date: val('date'), campaign: val('campaign'), adset: val('adset'), creative: val('creative'), hook: val('hook'), product: val('product') || '—', affiliateLink: val('link'), spend: num('spend'), commissionD0: num('d0'), commissionD1: num('d1'), commissionD3: num('d3'), commissionD7: num('d7'), clicks: parseInt(val('clicks'), 10) || 0, note: val('note'), source: editing?.source || 'local', synced: false
    });
  ...
  async function updateEntry(entry) {
    ...
    entries = entries.map((item) => {
      if ((item.rowId || item.id) === id) { found = true; return entry; }
      return item;
    });
    sheets = sheets.map((item) => {
      if ((item.rowId || item.id) === id) { found = true; return { ...entry, source: 'sheets', synced: true }; }
      return item;
    });
  ...
  function beginEdit(entry) {
    ...
    setVal('campaign', editing.campaign); setVal('adset', editing.adset); setVal('creative', editing.creative); setVal('hook', editing.hook); setVal('product', editing.product === '—' ? '' : editing.product); setVal('link', editing.affiliateLink); setVal('spend', editing.spend); setVal('d0', editing.commissionD0); setVal('d1', editing.commissionD1); setVal('d3', editing.commissionD3); setVal('d7', editing.commissionD7); setVal('clicks', editing.clicks); setVal('note', editing.note);
  ...
  function cancelEdit(clear = true) { editing = null; $('btn-add').textContent = '+ Tambah & Sync'; $('btn-cancel-edit').style.display = 'none'; $('edit-banner').classList.remove('show'); if (clear) clearForm(); }
  function clearForm() { ids.forEach((id) => $(id).value = ''); }
  ...
  async function retry() {
    ...
    for (const entry of list) {
      try { await post(norm(entry)); entry.synced = true; ok++; } catch {}
    }
    save();
    render();
  ...
  async function del(entry, source) {
    ...
    const id = entry.rowId || entry.id;
    if (source !== 'sheets') { entries = entries.filter((item) => (item.rowId || item.id) !== id); save(); }
    if (source === 'sheets') { sheets = sheets.filter((item) => (item.rowId || item.id) !== id); localStorage.setItem(SHEETS_KEY, JSON.stringify(sheets)); }
    render();
  ...
  function filtered() {
    ...
    if (status) data = data.filter((entry) => entry.status === status);
    return data;
  }
  ...
  function statusDashboard(data) {
    if (!data.length) {
      $('status-dashboard').innerHTML = '';
      return;
    }
    const stats = STATUSES.map((status) => {
      const rows = data.filter((entry) => entry.status === status);
      const spend = rows.reduce((sum, entry) => sum + entry.spend, 0);
      const commission = rows.reduce((sum, entry) => sum + entry.totalCommission, 0);
      const profit = commission - spend;
      return { status, rows, spend, commission, profit };
    });
    $('status-dashboard').innerHTML = stats.map((item) => {
      const clsName = cls(item.status);
      return `<div class="status-card ${clsName}"><div class="status-top"><span class="status-name">${item.status}</span><span class="status-count">${item.rows.length}</span></div><div class="status-grid"><span>Spend<b>${short(item.spend)}</b></span><span>Komisi<b>${short(item.commission)}</b></span><span>Profit<b class="${item.profit >= 0 ? 'status-profit-pos' : 'status-profit-neg'}">${item.profit >= 0 ? '+' : ''}${short(item.profit)}</b></span><span>K/S<b>${item.spend ? (item.commission / item.spend).toFixed(2) + 'x' : '—'}</b></span></div></div>`;
    }).join('');
  }
  ...
  function table(data) {
    if (!data.length) {
      $('table').innerHTML = '<div class="empty">Belum ada data. Tambahkan data di atas atau ubah filter.</div>';
      return;
    }
    ...
    if (toggle) toggle.onclick = () => { showAll = !showAll; table(filtered()); };
  }
  ...
  function draw(data) {
    ...
    const dataset = { labels, datasets: [ { label: 'Spend', data: spend, borderColor: '#2d5be3', backgroundColor: 'rgba(45,91,227,.06)', fill: true, tension: .35 }, { label: 'Komisi', data: commission, borderColor: '#1a7a52', backgroundColor: 'rgba(26,122,82,.06)', fill: true, tension: .35 }, { label: 'Profit/Rugi', data: profit, borderColor: profit[profit.length - 1] < 0 ? '#b83232' : '#1a7a52', fill: true, tension: .35, borderDash: [5, 3] } ] };
    if (chart) { chart.data = dataset; chart.update(); return; }
    chart = new Chart($('chart').getContext('2d'), { type: 'line', data: dataset, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: (value) => short(value).replace('Rp ', '') } } } } });
  }
  ...
  function exportCSV() {
    ...
    link.download = 'tracker-iklan-shopee-v16.csv';
    link.click();
